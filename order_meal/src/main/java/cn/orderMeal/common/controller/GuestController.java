...
//	}
	...
	public void applogin() throws WxErrorException {
		...
		setSessionAttr(SessionConst.OPENID, sessionInfo.getOpenid());
		setSessionAttr(SessionConst.SESSION_KEY, sessionInfo.getOpenid());
		Guest guest = getGuestByOpenId(sessionInfo.getOpenid());
		if (null == guest){
			Guest saveGuest = new Guest().setOpenid(sessionInfo.getOpenid());
			guestService.save(saveGuest);
			setSessionAttr(SessionConst.GUEST_INFO, saveGuest);
		} else{
			setSessionAttr(SessionConst.GUEST_INFO, guest);
		}
		renderJson(AjaxJson.success().setData(getSession().getId()));
	}
	...
	public void currentUserInfo(){
		WeixinUserInfo weixinUserInfo = getSessionAttr(SessionConst.WEIXIN_USERINFO);
	...
	public void updateGuestInfo() throws UnsupportedEncodingException, IOException{
		...
		Guest sessionGuest = getSessionAttr(SessionConst.GUEST_INFO);
		guest.setId(sessionGuest.getId());
		guest.setOpenid(sessionGuest.getOpenid());
		if (guestService.update(guest)){
			getSessionAttr(SessionConst.GUEST_INFO, guest);
